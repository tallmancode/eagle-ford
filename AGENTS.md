...
- `page_view` — `{ event, page_path }` on route changes including query-string updates (`GTMPageView`). Thank-you slugs (`/sales-form-submitted`, `/service-form-submitted`) are excluded — conversion `page_view` fires once from `ThankYouGate` after a legitimate form redirect only.
- `enquiry_submitted` — `{ event, form_name, form_id, department, submission_id, gclid?, … }` on successful form submit (`FormBlockClient` / `pushEnquirySubmitted`). **`form_name`** comes from the CMS **Analytics form name** select (`forms.form_name`); title matching is fallback-only. **`form_id`** comes from **External ID** (`forms.external_id`). Both must use **snake_case underscores** (never dots). Unresolved forms log a console warning and skip the event.
...
- Last-touch ad attribution (`gclid` / `_gcl_aw` cookie / UTMs) captured client-side (`eagle-ford:attribution`, **90-day TTL** with explicit `expiresAt`) via `AttributionCapture` (Suspense-wrapped in layout), stored on form-submissions and forwarded to Motor City LMS via site-form-leads. Expired gclids are cleared and not sent to GTM. A new ad click overwrites the stored gclid and resets the window.
- Forms: set **Analytics form name** and **External ID** on each enquiry form in admin (e.g. `general_enquiry`, `sell_your_car`) — underscores only, unique External ID, sent to GTM as `form_name` / `form_id`.
