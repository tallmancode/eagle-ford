...
describe('fetchStock sentry reporting', () => {
  ...
  it('does not capture Sentry for circuit-open fail-fast', async () => {
    ...
    await expect(fetchStock({ bodyType: 'suv' })).rejects.toMatchObject({
      code: CIRCUIT_OPEN_CODE,
    })
  ...
  it('captures Sentry for upstream 502 after retries by default', async () => {
    const { fetchMotorCityJson } = await import('@/lib/motor-city-stock/fetchMotorCity')
    vi.mocked(fetchMotorCityJson).mockRejectedValue(
  ...
  it('skips Sentry when reportToSentry is false (optional UI soft-fail)', async () => {
    const { fetchMotorCityJson } = await import('@/lib/motor-city-stock/fetchMotorCity')
    vi.mocked(fetchMotorCityJson).mockRejectedValue(
      new MotorCityStockError('Stock API request failed with status 502', 502, {
        code: 'HTTP_502',
    ...
    await expect(fetchStock({ bodyType: 'suv', reportToSentry: false })).rejects.toMatchObject({
      status: 502,
    })
